...
['https://www.googleapis.com/auth/drive.file'], // Cập nhật scope
);
...
scopes: [
'https://www.googleapis.com/auth/drive.readonly', // Quyền đọc tệp
],
);
...
// Gọi API để liệt kê các tệp ảnh (JPEG hoặc PNG)
...
if (files.isEmpty) {
_showErrorSnackBar('Không tìm thấy ảnh trong Google Drive');
return;
}
...
setState(() {
_imagePath = selectedFile['webContentLink']; // Lưu URL tải xuống
_image = null; // Không lưu tệp cục bộ
_aiResult = null;
});

// Giả lập nhận diện AI
await Future.delayed(const Duration(seconds: 2));
if (mounted) {
setState(() {
_aiResult = 'Rau củ quả (Độ tin cậy: 92%)';
...
_showErrorSnackBar('Lỗi khi lấy danh sách ảnh: ${response.body}');
}
} catch (e) {
...
_showErrorSnackBar('Lỗi khi đặt quyền cho tệp');
}
} catch (e) {
print('Lỗi đặt quyền: $e');
_showErrorSnackBar('Lỗi đặt quyền: $e');
}
}
...
// Sử dụng _imagePath trực tiếp làm imageUrl nếu ảnh từ Google Drive
String? imageUrl;
if (_image != null) {
// Nếu chọn ảnh cục bộ, tải lên Google Drive
imageUrl = await _uploadToGoogleDrive(_imagePath!);
if (imageUrl == null) {
throw Exception('Không thể tải ảnh lên Google Drive');
}
} else {
// Nếu chọn từ Google Drive, sử dụng _imagePath
imageUrl = _imagePath;
}
