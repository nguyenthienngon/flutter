...
class _MealPlanScreenState extends State<MealPlanScreen> with TickerProviderStateMixin {
  ...
  void _initializeAnimations() {
    ...
    _slideAnimation = Tween<double>(begin: 30.0, end: 0.0).animate(CurvedAnimation(parent: _animationController, curve: Curves.easeOut));
    _headerSlideAnimation = Tween<Offset>(begin: const Offset(0, -1), end: Offset.zero).animate(CurvedAnimation(parent: _headerAnimationController, curve: Curves.elasticOut));
  ...
  Future<void> _loadFavoriteRecipes() async {
    try {
      final response = await _httpClient.get(Uri.parse('$_ngrokUrl/get_favorite_recipes?userId=${widget.userId}')).timeout(const Duration(seconds: 30));
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        setState(() => _favoriteRecipeIds = (data['recipes'] as List<dynamic>?)?.map((r) => r['recipeId'].toString()).toSet() ?? {});
  ...
  Future<bool> _checkApiQuota() async {
    try {
      final response = await _httpClient.get(Uri.parse('$_ngrokUrl/check_quota?userId=${widget.userId}')).timeout(const Duration(seconds: 30));
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        return data['quotaAvailable'] == true;
      }
      return false;
    } catch (e) {
      _logger.e('Lỗi kiểm tra quota API: $e');
      return false;
    }
  }
  ...
  Future<void> _generateWeeklyMealPlan() async {
    ...
    final quotaAvailable = await _checkApiQuota();
    if (!quotaAvailable) {
      _showErrorSnackBar('Đã vượt quá giới hạn quota API. Vui lòng thử lại sau.');
      return;
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final processedWeek = mealPlan.map((day, dayData) {
          ...
          meals.sort((a, b) => (int.tryParse(a['readyInMinutes'] ?? '0') ?? 0)
              .compareTo(int.tryParse(b['readyInMinutes'] ?? '0') ?? 0));
        ...
        _showSuccessSnackBar('Đã tạo kế hoạch ${_timeFrameTranslations[_selectedTimeFrame]} với ${mealPlan.values.fold<int>(0, (sum, e) => sum + (e['meals'] as List?)!.length ?? 0)} công thức!');
      } else if (response.statusCode == 429) {
        _showErrorSnackBar('Đã vượt quá giới hạn quota API. Vui lòng thử lại sau.');
        setState(() => _loading = false);
      } else {
        _showErrorSnackBar('Lỗi khi tạo kế hoạch: ${response.body}');
        setState(() => _loading = false);
      }
    } catch (e) {
      _logger.e('Lỗi tạo kế hoạch: $e');
      _showErrorSnackBar('Lỗi khi tạo kế hoạch: $e');
      setState(() => _loading = false);
    }
  }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final newMeals = (data['recipes'] as List<dynamic>).map((r) => {
          ...
          'nutrients': r['nutrients'] ?? {'calories': 'N/A', 'carbohydrates': 'N/A', 'fat': 'N/A', 'protein': 'N/A'},
          'isFavorite': true,
          'timeSlot': 'Suggested',
        }).toList();
        setState(() {
      ...
      _showErrorSnackBar('Lỗi khi gợi ý công thức: $e');
      setState(() => _loading = false);
    }
  }
  ...
  Future<void> _syncMealPlanToCalendar() async {
    ...
    if (_googleUser == null || await _googleSignIn.isSignedIn() == false) {
      _showErrorSnackBar('Vui lòng đăng nhập Google để đồng bộ.');
      await showDialog(
        context: context,
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _showErrorSnackBar('Lỗi khi đồng bộ với Google Calendar: ${response.body}');
      }
    } catch (e) {
      _logger.e('Lỗi đồng bộ Google Calendar: $e');
      _showErrorSnackBar('Lỗi khi đồng bộ Google Calendar: $e');
    } finally {
      setState(() => _loading = false);
    }
  }
  ...
  Future<void> _toggleFavorite(String recipeId, bool isFavorite) async {
    ...
    try {
      final url = isFavorite
          ? '$_ngrokUrl/delete_favorite_recipe'
          : '$_ngrokUrl/add_favorite_recipe';
      ...
      _showErrorSnackBar('Lỗi khi cập nhật công thức yêu thích: $e');
    } finally {
      setState(() => _loading = false);
    }
  }
  ...
  Future<void> _addToShoppingList(Map<String, dynamic> recipe) async {
    ...
    try {
      ...
      _showErrorSnackBar('Lỗi khi thêm vào giỏ hàng: $e');
    } finally {
      setState(() => _loading = false);
    }
  }
                  ...
                  if ((recipe['ingredientsMissing'] as List<dynamic>? ?? []).isNotEmpty)
                    Container(
                      ...
                      child: ElevatedButton.icon(
                        ...
                        label: Text(
                          _loading ? 'Đang thêm...' : 'Thêm nguyên liệu vào giỏ hàng',
  ...
  Widget _buildRecipeTile(Map<String, dynamic> recipe) {
    _logger.i('Đang hiển thị công thức: $recipe');
      ...
      child: Container(
        ...
        child: ListTile(
          ...
          leading: Container(
            ...
            child: recipe['image'] != null && recipe['image'].isNotEmpty
              ...
              child: Image.network(
                recipe['image'],
                fit: BoxFit.cover,
                errorBuilder: (context, error, stackTrace) => Icon(Icons.restaurant, color: primaryColor),
              ),
            )
          ...
          trailing: IconButton(
            ...
            onPressed: () {
              _logger.i('Nhấn nút tim trong _buildRecipeTile, recipeId: ${recipe['id']}, isFavorite: ${recipe['isFavorite'] ?? false}');
              setState(() {
                recipe['isFavorite'] = !(recipe['isFavorite'] ?? false);
              });
              _toggleFavorite(recipe['id'].toString(), !(recipe['isFavorite'] ?? false));
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: ElevatedButton.icon(
                        ...
                        label: Text(
                          _loading ? 'Đang tạo...' : 'Tạo Kế Hoạch',
                          style: const TextStyle(
                            fontSize: 16,
                            fontWeight: FontWeight.bold,
                            color: Colors.white,
                    ...
                    if (_favoriteRecipeIds.isNotEmpty)
                      Container(
                        ...
                        child: ElevatedButton.icon(
                          ...
                          label: Text(
                            _loading ? 'Đang gợi ý...' : 'Gợi ý dựa trên yêu thích',
                            style: const TextStyle(
                              fontSize: 16,
                              fontWeight: FontWeight.bold,
                              color: Colors.white,
                    ...
                    if (_mealPlanId != null) ...[
                      ...
                      Container(
                        ...
                        child: ElevatedButton.icon(
                          ...
                          label: Text(
                            _loading ? 'Đang đồng bộ...' : 'Đồng bộ với Google Calendar',
                            style: const TextStyle(
                              fontSize: 16,
                              fontWeight: FontWeight.bold,
                              color: Colors.white,
